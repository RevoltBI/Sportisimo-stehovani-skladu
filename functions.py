...
def clustering_missmatch(result: pd.DataFrame, 
            ...
            for product2 in order_products:
                if product1 != product2:
                    if product1 in result_products and product2 in result_products:
                        #result obsahuje stlce Product_SK a Cluster
                        cluster1 = result.loc[result["Product_SK"] == product1].Cluster.item()
                        cluster2 = result.loc[result["Product_SK"] == product2].Cluster.item()
    ...
    result['New_Cluster'] = result.apply(lambda row : final_maping[row['Cluster']], axis = 1)
    del result['Cluster']
    return result
